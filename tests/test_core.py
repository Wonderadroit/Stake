...
def test_market_baseline_normalizes_opening_prices():
    ...
    assert result.loc[0, "AHHomeOpenProfit"] == pytest.approx(1.0)
    assert result.loc[0, "OUOverSettlement"] == 1.0
    assert result.loc[0, "OUOverOpenProfit"] == pytest.approx(1.0)
...
def test_market_baseline_preserves_missing_ah_line():
    frame = pd.DataFrame(
        {
            "Date": pd.to_datetime(["2024-01-01"]),
            "HomeTeam": ["A"],
    ...
    assert pd.isna(result.loc[0, "AHHomeOpenProfit"])
